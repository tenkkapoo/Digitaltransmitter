...
class Transmitter(thesdk):
    ...
    def main(self):
        ...
        def interpolator(time, I, Q, f_c, BW):
            ...
            new_I=np.interp(newtime, time, I)
            new_Q=np.interp(newtime, time, Q)
            print(len(newtime))
            print(len(new_Q))
            print(len(new_I))
            return newtime, new_I, new_Q


        def cartesian(time, I, Q, f_c):
            w_c=2*math.pi*f_c
            output=I*np.cos(w_c*time)-Q*np.sin(w_c*time)
            """f, p=signal.welch(output, 2*(1000e6+100e6), nperseg=1024)
            plot.semilogy(f, p)
            plot.show()"""
            return output
        ...
        out=cartesian(time, I, Q, self.f_c)
        if self.par:
            self.queue.put(out)
