...
class Transmitter(thesdk):
    ...
    def __init__(self,*arg): 
        ...
        self.Inter_factor=16          # Interpolation factor. Minimum 
    ...
    def main(self):
        ...
        def interpolator(time, I, Q, Fs, Fs_baseband, BW):
            ...
            for i in range(0, int(np.log2(self.Inter_factor))): 
                ...
                Q=np.trim_zeros(Q)
                BW=int(BW) 
                b=signal.remez(int(61), [0, 0.24*Fs_inter, 0.27*Fs_inter, 0.5*Fs_inter], [1, 0], Hz=int(Fs_inter))
            ...
            b=signal.remez(int(211), [0, 0.98*BW/2, 1.45*BW/2, 0.5*Fs_inter], [1, 0], Hz=int(Fs_inter))
            ...
            print(Fs_baseband)
            print(Fs_inter)
            return newtime, np.real(I_fil.reshape(1, -1)[0]), np.real(Q_fil.reshape(1, -1)[0])


        def quantization(I, Q, bits):
            I_step_size=(max(I)-min(I))/(2**bits)
            Q_step_size=(max(Q)-min(Q))/(2**bits)
            I_quantized=I_step_size*np.floor((I/I_step_size)+2**-1)
            Q_quantized=Q_step_size*np.floor((Q/Q_step_size)+2**-1)
        ...
        I, Q=quantization(inval[:, 1], inval[:, 2], self.bits)         #Input quantization
