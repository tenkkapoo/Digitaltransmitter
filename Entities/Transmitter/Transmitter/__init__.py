...
class Transmitter(thesdk):
    ...
    def main(self):
        ...
        def interpolator(time, I, Q, Fs, BW):
            ...
            BW=int(BW) 
            b=signal.remez(int(127), [0, 0.98*BW/2, BW*1.3/2, 0.5*Fs], [1, 0], Hz=int(Fs))
            ...
            newtime=np.arange(0, len(I_fil))/Fs
        ...
        def cartesian(time, I, Q, f_c, bits):
            w_c=2*math.pi*f_c
            output=np.multiply(np.real(I), np.cos(w_c*time))-np.multiply(np.real(Q), np.sin(w_c*time))
