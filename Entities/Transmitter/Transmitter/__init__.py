...
class Transmitter(thesdk):
    ...
    def __init__(self,*arg): 
        ...
        self.proplist = ['f_c', 'Fs', 'Inter_factor', 'amplitude_bits', 'phase_bits', 'baseband_bits' 'transmittertype'];    # Properties that can be propagated from parent
    ...
    def main(self):
        ...
        def multileveloutphasing(time, I, Q, f_c, amplitude_bits, phase_bits):
            w_c=2*math.pi*f_c
            r=np.sqrt(I**2+Q**2)
            A_MAX=2
            A_MO=np.ceil(r*A_MAX)
            phi1=np.arctan2(Q, I)+np.arccos(r*A_MAX/A_MO)
            phi1_step_size=(max(phi1)-min(phi1))/(2**phase_bits-1)
            phi1_quantized=phi1_step_size*(np.floor(phi1/phi1_step_size)+1/2)
            phi2=np.arctan2(Q, I)-np.arccos(r*A_MAX/A_MO)
            phi2_step_size=(max(phi2)-min(phi2))/(2**phase_bits-1)
            phi2_quantized=phi2_step_size*(np.floor(phi2/phi2_step_size)+1/2)
            S1=1/2*np.cos(w_c*time+phi1_quantized)
            S2=1/2*np.cos(w_c*time+phi2_quantized)
        ...
        if self.transmittertype=="multileveloutphasing":
            self.IOS.Members['Z'].Data=multileveloutphasing(time, I, Q, self.f_c, self.amplitude_bits, self.phase_bits)
        if self.par:
            self.queue.put(out)
