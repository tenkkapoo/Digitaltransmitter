...
class transmitter_testbench(thesdk):
    ...
    def main(self):
        ...
        def ACPR_sweep():
            ...
            for d.transmittertype in types:
                ACLRarray_1ADJC=[]
                ACLRarray_2ADJC=[]
                print(d.transmittertype)
                for d.amplitude_bits in range(2, 14, 2):
                    d.run()
            ...
            plot.xlabel("Bitit")
            plot.ylabel("ACLR ensimmäinen viereinen kanava (dB)")
            plot.legend()
            plot.show()
        ...
        d.phase_bits=12
        d.Inter_factor=128
        d.transmittertype="multileveloutphasing"
        d.run()
        r.init()
        r.Fc=-700e6
        r.Rs_sim=d.Fs
