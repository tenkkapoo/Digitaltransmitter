...
class transmitter_testbench(thesdk):
    ...
    def main(self):
        ...
        def ACPR_bit_sweep():
            ...
            d.phase_bits=12
            d.Inter_factor=128
            ACLRdict={"cartesian": 0, "polar": 0, "outphasing": 0, "multileveloutphasing": 0}
            ...
            for d.transmittertype in types:  #Loop for amplitude-bits
                ...
                for d.amplitude_bits in range(2, 13, 2):
                    d.run()
            ...
            bits=np.arange(2, 13, 1)
            plot.plot(bits, ACLRdict["cartesian"], label="Karteesinen")
            plot.plot(bits, ACLRdict["polar"], label="Polaarinen")
            plot.plot(bits, ACLRdict["outphasing"], label="Poisvaiheistus")
            plot.plot(bits, ACLRdict["multileveloutphasing"], label="Monitasoinen poisvaiheistus")
            plot.xlabel("Bitit")
            plot.ylabel("ACLR_1 (dB)")
        ...
        def EVM_bit_sweep():
            ...
            for d.transmittertype in types:  #Loop for amplitude-bits
                EVMarray=[]
                print(d.transmittertype)
                for d.amplitude_bits in range(2, 13, 1):
                    d.run()
                    r.init()
                    r.Fc=-700e6
                    r.Rs_sim=d.Fs
                    ...
                    s_source.run_dem()
                    s_source.run_EVM()
                    EVMarray.append(s_source.EVM[0][0])
                EVMdict[d.transmittertype]=EVMarray
            bits=np.arange(2, 13, 1)
            plot.plot(bits, EVMdict["cartesian"], label="Karteesinen")
            plot.plot(bits, EVMdict["polar"], label="Polaarinen")
            plot.plot(bits, EVMdict["outphasing"], label="Poisvaiheistus")
            plot.plot(bits, EVMdict["multileveloutphasing"], label="Monitasoinen poisvaiheistus")
            plot.xlabel("Bitit")
            plot.ylabel("EVM (%)")
            plot.xticks(bits, bits)
            plot.legend()
